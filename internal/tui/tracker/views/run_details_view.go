...
// RenderRunDetailsView renders the jobs ↔ steps two-pane view
func RenderRunDetailsView(
	...
	if loadingJobs {
		leftPaneContent = helpers.RenderPaneLoader(spinnerView, "Loading jobs...", leftContentWidth-2, paneHeight)
		rightPaneContent = helpers.RenderPaneLoader(spinnerView, "Loading steps...", rightPaneWidth-2, paneHeight)
	} else {
	...
	leftPane := helpers.RenderPaneWithTitle("Jobs", leftPaneContent, leftPaneWidth, paneHeight, focusPane == 0)
	rightPane := helpers.RenderPaneWithTitle("Steps", rightPaneContent, rightPaneWidth, paneHeight, focusPane == 1)

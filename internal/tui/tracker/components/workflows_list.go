...
// RenderWorkflowsHeader renders the header for the workflows pane
...
// RenderWorkflowsList renders the workflows list content for the viewport
func RenderWorkflowsList(
	...
	for i, wf := range workflows {
		...
		// Format the line with right-aligned pin indicator if pinned
		var displayName string
		maxNameLen := max(paneWidth-4, 0)

		if isPinned {
			displayName = helpers.Truncate(name, maxNameLen)
			padding := max(paneWidth-len(displayName)-5, 0)
			displayName = " " + displayName + strings.Repeat(" ", padding) + " " + pinIndicator + " "
		} else {
			displayName = helpers.Truncate(name, maxNameLen)
